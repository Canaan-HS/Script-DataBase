...
b={},c=[];const d=new Set("mp4 mov avi mkv flv wmv webm mpg mpeg m4v ogv 3gp asf ts vob rm rmvb m2ts f4v mts zip rar 7z gz bz2 .pg jpeg png gif webp bmp tiff tif svg .eic heif raw ico psd mp3 aac flac wav ogg pdf".split(" "));return{async destroyWorker(){a&&(a.terminate(),a=null)},async file(e,h){const f=new Promise(async g=>{const k=await h.arrayBuffer();b[e]=new Uint8Array(k);g()});c.push(f);return f},async generateZip(e={},h){await Promise.all(c);return new Promise((f,g)=>{if(0===Object.keys(b).length)return g("Empty Data Error");
const k={};Object.entries(b).forEach(([l,p])=>{var r=l.split(".").pop().toLowerCase();r=d.has(r)?0:e.level||5;k[l]={data:p,level:r}});a.postMessage({filesWithOptions:k},Object.values(k).map(l=>l.data.buffer));a.onmessage=l=>{l=l.data;"progress"===l.type?h?.(l.loaded/l.total*100):"done"===l.type?(h?.(100),b={},c=[],f(new Blob([l.data],{type:"application/zip"}))):"error"===l.type&&(b={},c=[],g(l.error))}})}}},createNnetworkObserver:function(a={}){function b(){var m=Date.now();if(m-A<l)return y;A=m;
