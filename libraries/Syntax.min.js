...
!0,subtree:n=!0,childList:p=!0,attributes:u=!0,characterData:r=!1,timeoutResult:x=!1,root:z=document}=c||{};return new Promise((l,t)=>{const q=async function(){let y,v;if(e){let A;const F=()=>{(v=d(a,h))?(cancelAnimationFrame(A),clearTimeout(y),b?.(v),l(v)):A=requestAnimationFrame(F)};A=requestAnimationFrame(F);y=setTimeout(()=>{cancelAnimationFrame(A);x&&(b?.(v),l(v))},1E3*k)}else{const [A,F]=0<m?[K,m]:[L,g],M=new MutationObserver(A(()=>{if(v=d(a,h))M.disconnect(),clearTimeout(y),b?.(v),l(v)},F));
M.observe(z,{subtree:n,childList:p,attributes:u,characterData:r});y=setTimeout(()=>{M.disconnect();x&&(b?.(v),l(v))},1E3*k)}};f&&"hidden"===document.visibilityState?document.addEventListener("visibilitychange",()=>q(),{once:!0}):q()})}function U(a,b,c,d){let e;return null!=c?V[B(c)](b,a,c):(e=b.getItem(a),void 0!=e?V[B(JSON.parse(e))](b,e):d)}function K(a,b){let c=0;return(...d)=>{const e=Date.now();e-c>=b&&(c=e,a(...d))}}function L(a,b){let c=null;return(...d)=>{clearTimeout(c);c=setTimeout(function(){a(...d)},
b)}}function W(a){a=new Blob([a],{type:"application/javascript"});const b=URL.createObjectURL(a),c=new Worker(b),d=c.terminate;c.terminate=function(){d.call(c);URL.revokeObjectURL(b)};return c}const Z=new DOMParser,B=a=>Object.prototype.toString.call(a).slice(8,-1);[Document.prototype,Element.prototype].forEach(a=>{a.$q=function(b){return w(this,b,!1)};a.$qa=function(b){return w(this,b,!0)}});const N={$text(a=null){return null===a?this?.textContent?.trim():this.textContent=a?.trim()??""},$copy(a=
...
1E3,Network_Check_Interval:f=1E4,EMA_ALPHA:n=.3,ADJUSTMENT_FACTOR:p=.25,History_Size:u=10}=a||{},{responseHistory:r=[],lastNetworkCheck:x=0,networkCondition:z="normal"}={};return function(l,t,q=null,y=0){l=Date.now()-l;r.push(l);r.length>u&&r.shift();g=!g||1>=r.length?l:n*l+(1-n)*g;g=Math.max(20,Math.min(2E3,g));const {delayFactor:v,threadFactor:A}=m[b()];l/=g;t=Math.max(y,Math.min(t*(1+(l-1)*p*v),c));return null!==q?[Math.round(t),Math.round(Math.max(d,Math.min(q*(1-(l-1)*p*A),e)))]:Math.round(t)}},
