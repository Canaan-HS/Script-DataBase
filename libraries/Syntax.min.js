...
return function(...a){const b={};for(const c of a)c&&Object.defineProperties(b,Object.getOwnPropertyDescriptors(c));return b}({get sX(){return window.scrollX},get sY(){return window.scrollY},get iW(){return window.innerWidth},get iH(){return window.innerHeight},get platform(){const a={desktop:!1,mobile:!1};window.matchMedia?.("(pointer: coarse)")?.matches||window.matchMedia?.("(hover: none)")?.matches?a.mobile=!0:/Android|iPhone|iPad|iPod|webOS|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)?
...
c(n),15)};g.forEach(n=>a.addEventListener(n,h,e))}d?.(!0)}catch{d?.(!1)}},onEvent:async function(a,b,c,e={}){const {mark:d,...g}=e;"string"===typeof a&&(a=y(document,a));e=d??a;const l=C.get(e);l?.has(b)||(a.addEventListener(b,c,g),l||C.set(e,new Map),C.get(e).set(b,{element:a,type:b,listener:c}))},offEvent:async function(a,b,c){if(null==a&&null==b&&null==c){for(const e of C.values())for(const {element:d,type:g,listener:l}of e.values())d.removeEventListener(g,l);C.clear()}else if(a=c??"string"===
typeof a?y(document,a):a,c=C.get(a))if(b=c.get(b)){const {element:e,type:d,listener:g}=b;e.removeEventListener(d,g);C.get(a).delete(d)}else{for(const {element:e,type:d,listener:g}of c.values())e.removeEventListener(d,g);C.delete(a)}},onUrlChange:function(a,b=15){function c(m){clearTimeout(d);l||"urlchange"!==m||(l=!0);d=setTimeout(()=>{l&&e(!1,!0);a({type:m,url:location.href,domain:location.hostname})},Math.max(15,b))}function e(m=!0,k=!1){k&&g||(clearTimeout(d),history.pushState=h,history.replaceState=
n,window.removeEventListener("popstate",f.popstate),window.removeEventListener("hashchange",f.hashchange),m&&window.removeEventListener("urlchange",f.urlchange),g=!0)}let d,g=!1,l=!1;const h=history.pushState,n=history.replaceState,f={urlchange:()=>c("urlchange"),popstate:()=>c("popstate"),hashchange:()=>c("hashchange")};window.addEventListener("urlchange",f.urlchange);window.addEventListener("popstate",f.popstate);window.addEventListener("hashchange",f.hashchange);history.pushState=function(){h.apply(this,
arguments);c("pushState")};history.replaceState=function(){n.apply(this,arguments);c("replacestate")};return{off:e}},log:function(...a){if(0!==a.length){var b={},c=a,e={dev:!0,group:null,collapsed:!0};if(1<a.length){const f=a[0],m=a.at(-1),k=f?.constructor===Object,r=m?.constructor===Object;if(r&&!k)b=m,c=a.slice(0,-1);else if(k&&!r)b=f,c=a.slice(1);else if(k&&r){const p=new Set(Object.keys(e));b=Object.keys(f).filter(q=>p.has(q)).length;Object.keys(m).filter(q=>p.has(q)).length>b?(b=m,c=a.slice(0,
-1)):(b=f,c=a.slice(1))}}var {dev:d,group:g,collapsed:l}={...e,...b};if(!d||0===c.length)return new Proxy({},{get(){}});var h=f=>{"count"===f?T.count(c[0]):(f=T[f]||T.log,g?(l?console.groupCollapsed(g):console.group(g),f(...c),console.groupEnd()):f(...c))},n=setTimeout(()=>h("log",c));return new Proxy({},{get(f,m){clearTimeout(n);h(m,c)}})}},delHead:async function(a){if(null==a){for(var b of E.values())b.remove();E.clear()}else if(b=E.get(a))b.remove(),E.delete(a)},observer:async function(a,b,c={},
e){const {mark:d="",debounce:g=0,throttle:l=100,subtree:h=!0,childList:n=!0,attributes:f=!0,characterData:m=!1}=c||{};if(d&&(c=U.get(d))){if(c.target===a)return;c.ob.disconnect()}const [k,r]=0<g?[L,g]:[K,l];c={subtree:h,childList:n,attributes:f,characterData:m};const p=new MutationObserver(k(()=>{b()},r));p.observe(a,c);d&&U.set(d,{target:a,ob:p});e?.({ob:p,op:c})},waitEl:X,openDB:async function(a="StorageDB",b=1,c){return new Promise((e,d)=>{const g=indexedDB.open(a,b),l=a.replace(/db$/i,"").toLowerCase();
g.onupgradeneeded=h=>{h=h.target.result;h.objectStoreNames.contains(l)||h.createObjectStore(l)};g.onsuccess=h=>{async function n(m){return new Promise((k,r)=>{const p=f.transaction(l,"readwrite").objectStore(l).delete(m);p.onsuccess=()=>k(p.result);p.onerror=()=>r(p.error)})}const f=h.target.result;e({set:async function(m,k,{compress:r=!0,expireStr:p}={}){const q={};r?(G??=Q(c),r=A(k),q.type=r,q.data=await G.compress(aa[r]?.(k)??k,{base64:!1}),q.compressed=!0):q.data=k;if(k=Z(p))q.expire=k;return new Promise((w,
t)=>{const u=f.transaction(l,"readwrite").objectStore(l).put(q,m);u.onsuccess=()=>w(u.result);u.onerror=()=>t(u.error)})},get:async function(m,k,r=!1){const p=f.transaction(l,"readonly").objectStore(l),q=await new Promise((w,t)=>{const u=p.get(m);u.onsuccess=()=>w(u.result);u.onerror=()=>t(k)});if(null==q)return k;try{const w=q instanceof Object;if(w&&q.expire&&Date.now()>1E3*q.expire)return n(m),k;let t;if(w){if(t=q.data??q,q.compressed){G??=Q(c);t=await G.decompress(t);const u=q.type||A(t);t=ba[u]?.(t)??
t}}else t=q;r&&n(m);return t}catch{return q}},del:n,destroy:function(){f&&(f.close(),indexedDB.deleteDatabase(a),G.destroyWorker(),setTimeout(()=>G=null,100))}})};g.onerror=()=>d(g.error)})},throttle:K,debounce:L,scopeParse:function(a,b){if("string"!==typeof a||""===a.trim())return b;const c=b.length;let e=!1;const d=new Set,g=new Set,l=(h,n,f)=>{const m=h<=n;for(;m?h<=n:h>=n;m?h++:h--)f.add(h)};for(const h of a.split(/\s*[\.,|/]\s*/))if(/^(!|-)?\d+(~\d+|-\d+)?$/.test(h)){a=h.slice(1);const n=/^[!-]/.test(h),
f=/[~-]/.test(a),[m,k]=n?[g,a]:(e=!0,[d,h]),[r,p]=f?k.split(/-|~/):[k,k];r===p?m.add(+r-1):l(+r-1,+p-1,m)}return(e?[...d]:[...b.keys()]).filter(h=>!g.has(h)&&h<c&&0<=h).sort((h,n)=>h-n).map(h=>b[h])},createWorker:P,formatTemplate:function(a,b){if("Object"!==A(a))return"Template must be an object";a=Object.fromEntries(Object.entries(a).map(([c,e])=>[c.toLowerCase(),e]));return"String"===A(b)?b.replace(/\{\s*([^}\s]+)\s*\}/g,(c,e)=>fa.process(a,e)):"Object"===A(b)?Object.entries(b).map(([c,e])=>fa.process(a,
