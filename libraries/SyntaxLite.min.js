...
g=!0)}let d,g=!1,h=!1;const n=history.pushState,q=history.replaceState,f={urlchange:()=>c("urlchange"),popstate:()=>c("popstate"),hashchange:()=>c("hashchange")};window.addEventListener("urlchange",f.urlchange);window.addEventListener("popstate",f.popstate);window.addEventListener("hashchange",f.hashchange);history.pushState=function(){n.apply(this,arguments);c("pushState")};history.replaceState=function(){q.apply(this,arguments);c("replacestate")};return{off:e}},log:function(...a){if(0!==a.length){var b=
{},c=a,e={dev:!0,group:null,collapsed:!0};if(1<a.length){const f=a[0],k=a.at(-1),m=f?.constructor===Object,r=k?.constructor===Object;if(r&&!m)b=k,c=a.slice(0,-1);else if(m&&!r)b=f,c=a.slice(1);else if(m&&r){const p=new Set(Object.keys(e));b=Object.keys(f).filter(l=>p.has(l)).length;Object.keys(k).filter(l=>p.has(l)).length>b?(b=k,c=a.slice(0,-1)):(b=f,c=a.slice(1))}}var {dev:d,group:g,collapsed:h}={...e,...b};if(!d||0===c.length)return new Proxy({},{get(){}});var n=f=>{"count"===f?S.count(c[0]):(f=
...
c.ob.disconnect()}const [m,r]=0<g?[L,g]:[K,h];c={subtree:n,childList:q,attributes:f,characterData:k};const p=new MutationObserver(m(()=>{b()},r));p.observe(a,c);d&&T.set(d,{target:a,ob:p});e?.({ob:p,op:c})},waitEl:W,openDB:async function(a="StorageDB",b=1,c){return new Promise((e,d)=>{const g=indexedDB.open(a,b),h=a.replace(/db$/i,"").toLowerCase();g.onupgradeneeded=n=>{n=n.target.result;n.objectStoreNames.contains(h)||n.createObjectStore(h)};g.onsuccess=n=>{async function q(k){return new Promise((m,
r)=>{const p=f.transaction(h,"readwrite").objectStore(h).delete(k);p.onsuccess=()=>m(p.result);p.onerror=()=>r(p.error)})}const f=n.target.result;e({set:async function(k,m,{compress:r=!0,expireStr:p}={}){const l={};r?(F??=P(c),r=H(m),l.type=r,l.data=await F.compress(Z[r]?.(m)??m,{base64:!1}),l.compressed=!0):l.data=m;if(m=Y(p))l.expire=m;return new Promise((x,t)=>{const u=f.transaction(h,"readwrite").objectStore(h).put(l,k);u.onsuccess=()=>x(u.result);u.onerror=()=>t(u.error)})},get:async function(k,
m,r=!1){const p=f.transaction(h,"readonly").objectStore(h),l=await new Promise((x,t)=>{const u=p.get(k);u.onsuccess=()=>x(u.result);u.onerror=()=>t(m)});if(null==l)return m;try{const x=l instanceof Object;if(x&&l.expire&&Date.now()>1E3*l.expire)return q(k),m;let t;if(x){if(t=l.data??l,l.compressed){F??=P(c);t=await F.decompress(t);const u=l.type||H(t);t=aa[u]?.(t)??t}}else t=l;r&&q(k);return t}catch{return l}},del:q,destroy:function(){f&&(f.close(),indexedDB.deleteDatabase(a),F.destroyWorker(),setTimeout(()=>
...
Info:e})}},runTime:function(a,{lable:b="Elapsed Time:",log:c=!0,format:e=!0,style:d="\u001b[1m\u001b[36m%s\u001b[0m"}={}){if(!a)return performance.now();a=e?`${((performance.now()-a)/1E3).toPrecision(3)}s`:performance.now()-a;return c?console.log(d,`${b} ${a}`):a},getDate:function(a){const b=new Date,c={year:b.getFullYear(),month:`${b.getMonth()+1}`.padStart(2,"0"),date:`${b.getDate()}`.padStart(2,"0"),hour:`${b.getHours()}`.padStart(2,"0"),minute:`${b.getMinutes()}`.padStart(2,"0"),second:`${b.getSeconds()}`.padStart(2,
"0")};return(e=>e.replace(/{([^}]+)}/g,(d,g)=>c[g]??"Error"))("string"===typeof a?a:"{year}-{month}-{date} {hour}:{minute}:{second}")},translMatcher:function(a,b=navigator.language,c="en-US"){return a[U[b]]??a[U[c]]??a[U["en-US"]]},regMenu:function(a,b={}){let {name:c="regMenu",index:e=1,reset:d=!1}=b||{};d&&([...D].map(g=>GM_unregisterMenuCommand(g)),D.clear());for(let [g,h]of Object.entries(a))a=`${c}-${e++}`,"function"===typeof h&&(h={func:h}),GM_registerMenuCommand(g,()=>{h.func()},{id:a,title:h.desc,
autoClose:h.close,accessKey:h.hotkey}),D.add(a)},unMenu:async function(a){null==a?([...D].map(b=>GM_unregisterMenuCommand(b)),D.clear()):Array.isArray(a)?a.forEach(b=>{D.delete(b)&&GM_unregisterMenuCommand(b)}):D.delete(a)&&GM_unregisterMenuCommand(a)},storageListen:function(a,b){a.forEach(c=>{if(!G.has(c)){const e=GM_addValueChangeListener(c,function(d,g,h,n){b({key:d,ov:g,nv:h,far:n})});G.set(c,e)}});return{off(c){const e=G.get(c);e&&(GM_removeValueChangeListener(e),G.delete(c))},offAll(){for(const c of G.values())GM_removeValueChangeListener(c);
G.clear()}}},sleep:a=>new Promise(b=>setTimeout(b,a)),domParse:a=>fa.parseFromString(a,"text/html")})})();